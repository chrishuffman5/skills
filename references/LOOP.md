...
{skill-name}/references/*.md    # If exists
```

**Extract:**
- Frontmatter (name, description, compatibility)
- Current trigger phrases
- Existing test prompts (if any)

**Token baseline:** Count tokens in SKILL.md (use `wc -w` for a rough word count, or a tokenizer if available). Store as baseline for comparison.
...
4. Contains trigger phrases ("WHEN:" etc.)
5. Has compatibility field (optional for Medium-High)
6. Optional fields preserved if present (`license`, `metadata`, `allowed-tools`)

**Output:** Invalid | Low | Medium | Medium-High | High

> If name validation fails, report as **Invalid** and fix before proceeding. A name like `my--skill` or `-my-skill` violates the spec.

### Step 3: SCAFFOLD (Conditional)

**Condition:** No trigger tests exist for this skill

**Action:** Create trigger test prompts

**Then define:**
1. `shouldTriggerPrompts` - 5+ prompts matching frontmatter triggers
...
> **Do NOT add "DO NOT USE FOR:" clauses.** They cause keyword contamination on Claude Sonnet — anti-triggers introduce the very keywords that trigger wrong-skill activation. Use positive routing with distinctive `WHEN:` phrases instead.
...
2. **shouldNotTriggerPrompts** (minimum 5):
   ...
   - Include adjacent domains that use similar keywords
   - Include related but different skills' territory

### Step 5b: VERIFY

**Action:** Run tests to ensure trigger prompts align with the description

**Expected outcome:**
- Trigger prompts are relevant to the skill's described purpose
- Anti-trigger prompts clearly belong to other skills or are unrelated
...
**Token estimation:** Count words in SKILL.md (as a rough proxy, 1 token ≈ 0.75 words). Use `wc -w` or a tokenizer if available.
...
**Capture:**
- Current token/word count
- Delta from start
- Optimization suggestions (for summary)

Token optimizations are captured but NOT automatically applied. The user decides whether to implement them or create an issue for follow-up.
...
1. Verify skill exists at the expected path
...
| Error rate | 0 failed API calls per workflow | Monitor logs during test runs |
